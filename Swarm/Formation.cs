...
// using MissionPlanner.Utilities;  // Removed to avoid Vector3 conflicts
...
namespace MissionPlanner.Swarm
{
    ...
    {
        private static readonly ProjNet.CoordinateSystems.GeographicCoordinateSystem _wgs84 = (GeographicCoordinateSystem)GeographicCoordinateSystem.WGS84;
    ...
    // --- PID Controller with σ-mod adaptive gains ---
    ...
    internal class LoadAttitudeController
    {
        ...
        private static readonly Matrix<double> Inertia = Matrix<double>.Build.DenseOfArray(
            new double[,] { { 0.021, 0.0, 0.0 }, { 0.0, 0.0187, 0.0 }, { 0.0, 0.0, 0.0397 } });
            ...
            out Matrix<double> nullspace)
        {
            ...
            var Wm = -(Inertia * omega + Cross(omega, Inertia * omega));
            var W = Vector<double>.Build.Dense(6);
            W.SetSubVector(0, 3, Wf); W.SetSubVector(3, 3, Wm);

            var pts = new List<Vector3Wrapper>(offsets.Values); int n = pts.Count;
            var Phi = Matrix<double>.Build.Dense(6, n);
            for (int i = 0; i < n; i++)
            {
                var orig = pts[i].V; float len = VectorUtils.Length(orig);
                var rvec = len > 0
                    ? new Vector3(orig.X * (SwarmConstants.CableLength / len),
                                  orig.Y * (SwarmConstants.CableLength / len),
                                  orig.Z * (SwarmConstants.CableLength / len))
                    : orig;
            ...
            var Tp = pinv * W; var lambda = Vector<double>.Build.Dense(n - 6);
            var T = Tp + nullspace * lambda;

            int idx = new List<MAVState>(offsets.Keys).IndexOf(follower);
            if (idx < 0 || idx >= T.Count) return Vector3Wrapper.Zero;
            var soff = offsets[follower].V;
            var so = VectorUtils.Normalize(new Vector3Wrapper(soff.X, soff.Y, soff.Z)).V;
            float alpha = 0.2f; float tension = (float)Math.Max(0, T[idx]);
            return new Vector3Wrapper(so.X * tension * alpha, so.Y * tension * alpha, so.Z * tension * alpha);
        }
        ...
        private Vector<double> Cross(Vector<double> a, Vector<double> b)
            => Vector<double>.Build.Dense(new[] { a[1] * b[2] - a[2] * b[1], a[2] * b[0] - a[0] * b[2], a[0] * b[1] - a[1] * b[0] });
    }
    ...
    internal class Formation : Swarm
    {
        ...
        private readonly Dictionary<MAVState, (PIDController roll, PIDController pitch)> pids = new Dictionary<MAVState, (PIDController, PIDController)>();
        private readonly LoadAttitudeController loadCtrl = new LoadAttitudeController();
        private PointLatLngAlt masterpos = new PointLatLngAlt(); private DateTime lastTime = DateTime.UtcNow;

        // Adaptive control constants
        private const float sigma = 0.1f, Kp0 = 1f, Ki0 = 0.1f, Kd0 = 0.01f, Kv0 = 1f, Gi = 0.1f, Gd = 0.01f;

        public void setOffsets(MAVState m, float x, float y, float z) => offsets[m] = new Vector3Wrapper(x, y, z);
        public Vector3Wrapper getOffsets(MAVState m) => offsets.TryGetValue(m, out var w) ? w : Vector3Wrapper.Zero;
        ...
        public override void SendCommand()
        {
            ...
            foreach (var port in MainV2.Comports) foreach (var mav in port.MAVlist)
                {
                    ...
                    if (!pids.ContainsKey(mav)) pids[mav] = (new PIDController(Kp0, Ki0, Kd0), new PIDController(Kv0, Ki0, Kd0));
                    ...
                    pidP.ClampLower(Kp0, Ki0, Kd0); pidV.ClampLower(Kv0, Ki0, Kd0);
                    float dPos = (posErr.V.X - pidP.lastError) / dt; float outP = pidP.Step(posErr.V.X, dPos, dt); pidP.lastError = posErr.V.X;
                    float dVel = (velErr.V.X - pidV.lastError) / dt; float outV = pidV.Step(velErr.V.X, dVel, dt); pidV.lastError = velErr.V.X;
                    ...
                    port.setPositionTargetGlobalInt(
                        ...
                        leaderVel.V.X, leaderVel.V.Y, leaderVel.V.Z, 0f, 0f);
                }
        }
    }
}